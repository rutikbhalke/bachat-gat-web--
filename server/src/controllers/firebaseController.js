...
async function deleteMember(req, res, next) { try { await db.collection('users').doc(req.params.id).set({ status: 'INACTIVE', isActive: false }, { merge: true }); res.json({ success: true, message: 'Member deactivated successfully.' }); } catch (err) { next(err); } }
